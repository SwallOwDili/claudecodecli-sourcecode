...
调研日期：2026-08-20。公开网页会继续更新，本页保存的是当日读到的主张及其与 `2.1.235` 的对应关系，不把当前文档发布日期倒推成本版本发布日期。
...
| session 可以继续、fork 并持久化 transcript | `Static` + `Probe` | resume 修复 UUID/parent/message graph，323188-323443；合法但不存在的 UUID 返回明确错误 | 本版会校验会话存在性，不会静默创建同 ID 新会话 |
...
| MCP 可动态提供工具 | `Static` + `Probe` | MCP tool list generation/refresh，491915-491964；无配置时 `mcp list` 返回固定文本 | 工具目录不是只在启动时静态冻结 |
...
| subagent 在独立上下文中工作并向主 Agent 返回结果 | `Static` + `Probe` | 默认 `maxTurns: 200`、`model: inherit`、`permissionMode: bubble`；`agents --json` 可运行 | 独立上下文带来容量和隔离，也带来额外 token/协调成本 |
...
Exit status: 1
Result: 本版区分“ID 合法”与“本地存在可恢复 transcript”
```
