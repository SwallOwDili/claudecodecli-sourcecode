...
const INTERNAL_ENTRYPOINTS = [
  ["--handle-uri <uri>", "OS protocol handler", 630130],
  ["--claude-in-chrome-mcp", "Chrome MCP child", 637899],
  ["--chrome-native-host", "Chrome native-messaging host", 637903],
  ["--computer-use-mcp", "Computer Use MCP child", 637907],
  ["--daemon-worker <kind>", "daemon registry worker", 637911],
  ["--bg-pty-host", "background PTY host", 637919],
  ["--bg-spare", "background spare worker", 637928],
  ["--preload", "preload worker", 637935],
];
...
async function main() {
  ...
  const inventory = {
    ...
    internalEntrypoints: INTERNAL_ENTRYPOINTS.map(([argv, owner, sourceLine]) => ({
      argv,
      visibility: "internal",
      owner,
      source: `reverse/javascript/cli.readable.js:${sourceLine}`,
      failureBoundary: "These are child-process or OS integration protocols, not ordinary user commands; invoking them directly may block or fail without the parent-owned environment.",
    })),
    caveats: [
