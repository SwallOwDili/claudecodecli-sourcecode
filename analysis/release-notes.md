...
| 13 | embedded grep 遇病态 pattern 消耗过高；`-m N` 与 `-A/-C` 的停止边界会截错 context | native macOS/Linux build 对病态模式 fast-fail，并在达到 match cap 后保留正确上下文 | 搜索失败更快，有限匹配的周边行可信 | **Release**；bundle 能定位 embedded search surface，但本仓库尚无精确 binary regression corpus，因此保持运行验证边界 |
