...
Treat `analysis/product-surface-evidence-map.md` as an architectural article, not an inventory directory. It must open with a concrete end-to-end task, use the four distinct Agent Loop lifecycle units (user turn, model iteration, API attempt and tool batch) as the runtime spine, then teach six fixed source-anchored cases: request-time Bash registration/scheduling, the layered context-governance stack including `/compact` precomputed hit/miss, split telemetry pipelines, typed per-object session/checkpoint/remote-reference recovery, a stateful Artifact publish route, and the native Voice bridge. Derive the C/Q/E/S/O control/request/execution/state/observation reading model from that spine and those cases, and state explicitly that it is an analyst-created navigation framework, not source-native modules or an official Anthropic architecture. For each case, distinguish candidate, registration/declaration, reachable consumer, gate, owned state, failure/recovery and post-side-effect Boundary. Only after that narrative may it expose the exhaustive 71-class inventory table, and that table must be collapsed by default. The appendix proves inventory completeness, stable ordering and reproducible classification; it must not claim exact ownership for every identifier or completed consumer tracing. Classify every row on three independent axes: extraction origin, ownership (`Product`/`Dependency`/`Mixed`/`Unresolved`) and proof level (`Candidate`/`Declaration`/`Callsite`/`Structured surface`/`Evidence substrate`). A whole-bundle AST callsite is not automatically first-party Product. Explicitly state that `Untraced/Inventory only` is unfinished client analysis, not an irrecoverable Boundary.

Give that article one governing thesis and make every case test it. The default thesis form is: the model owns proposals, the client owns authorization and the causal ledger, and external systems own completed effects. Teach these as three distinct state worlds before introducing any taxonomy. Frame the six cases as engineering contradictions rather than disconnected feature summaries: autonomy versus local authority, latency versus causal ordering, context economy versus history fidelity, continuity versus non-rollback, observability versus truth/privacy, and native integration versus source/ABI boundaries. Do not front-load a “top N features” table that repeats the rest of the article. Derive the version's technical character only after the cases, then introduce C/Q/E/S/O as a reading router.
