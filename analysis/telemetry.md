...
这意味着管理员开启 OTLP logs exporter 后，事件元数据可以默认离开本机，但普通 `user_prompt` event 的正文仍由单独 content gate 控制。反过来，一旦开启正文 gate，prompt 会进入管理员指定 collector；这不是“只写本地 debug log”。

### Raw API body 是独立的更高风险内容门

`OTEL_LOG_RAW_API_BODIES` 不复用 `OTEL_LOG_USER_PROMPTS`。静态主链在 [L232070-L232127](../reverse/javascript/cli.readable.js#L232070) 先解析三种模式：空/假值为 disabled，其他 truthy 值为 inline，`file:<dir>` 为 file。request 会保留 system/messages/tools 等请求对象，response 会聚合 assistant content；两者只把 `thinking` 和 `redacted_thinking` 内容替换为 `<REDACTED>`，不会因 `OTEL_LOG_USER_PROMPTS` 未开启而删掉 user message。

| 模式 | 客户端状态变化 | 精确二进制 Probe | 隐私/运维影响 |
| --- | --- | --- | --- |
| disabled（默认） | `KYp` 直接返回，不生成 raw-body event | 普通 OTLP logs 仍导出，但无 `api_request_body` / `api_response_body`，两个 marker 都未进 collector | 不代表 user-prompt/tool/assistant 等其他内容门也关闭 |
| inline | 经 `xB` 长度上限处理后，在 OTLP log attribute 写 `body`、`body_length` 和可选 `body_truncated` | collector 同时收到 request/response event，受控 request/response marker 都出现，即使 `OTEL_LOG_USER_PROMPTS` 未开启 | 完整上下文可能进入第三方 collector；这是独立于 prompt event 脱敏的显式 opt-in |
| `file:<dir>` | 正文 JSON 写入本地 request/response 文件，OTLP 只写 `body_ref`、`body_length` 和关联字段 | 本地出现 2 个 request 文件和 1 个 response 文件，两类 marker 在文件中；collector 只有 ref，无 marker | 目录由运行者指定；`writeFile` 没有额外声明 mode，依赖文件系统 umask/目录权限保护正文 |

三组 raw-body 运行分别登记为 `probe.telemetry-raw-api-default`、`probe.telemetry-raw-api-inline` 和 `probe.telemetry-raw-api-file`。五次 CLI 命令、五次 HTTP/JSON collector export 均 exit 0；报告只保留 marker 命中、文件类型/字节数和规范化命令，不提交真实 body 或本机路径。
...
本专题结构化主张包括 `telemetry.otel-gate-exporters`、`telemetry.content-redaction`、`public.otel-exporter-precedence`、`public.otel-prompt-redaction`、`probe.telemetry-otlp-redaction` 和三个 `probe.telemetry-raw-api-*`。Probe 的命令、input、literal output、exit status 和字段读法见 [精确二进制运行证据指南](runtime-probe-index.md)。
